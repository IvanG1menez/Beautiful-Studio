'use client';

import { Button } from '@/components/ui/button';
import { Loader2 } from 'lucide-react';
import { useEffect, useState } from 'react';

interface SSOConfig {
  google_sso_activo: boolean;
  autocreacion_cliente_sso: boolean;
}

export default function GoogleSSOButton() {
  const [ssoConfig, setSSOConfig] = useState<SSOConfig | null>(null);
  const [isLoading, setIsLoading] = useState(true);
  const [isAuthenticating, setIsAuthenticating] = useState(false);

  useEffect(() => {
    // Obtener configuración de SSO del backend
    const fetchSSOConfig = async () => {
      try {
        const API_URL = process.env.NEXT_PUBLIC_API_URL || 'http://127.0.0.1:8000/api';
        const response = await fetch(`${API_URL}/configuracion/sso/public/`, {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
          },
        });

        if (!response.ok) {
          console.error('Error al obtener configuración SSO:', response.statusText);
          setIsLoading(false);
          return;
        }

        const data: SSOConfig = await response.json();
        setSSOConfig(data);
      } catch (error) {
        console.error('Error al cargar configuración de SSO:', error);
      } finally {
        setIsLoading(false);
      }
    };

    fetchSSOConfig();
  }, []);

  const handleGoogleSSO = async () => {
    setIsAuthenticating(true);

    try {
      const API_URL = process.env.NEXT_PUBLIC_API_URL || 'http://127.0.0.1:8000/api';

      // Redirigir al endpoint de OAuth del backend
      // Social-auth-app-django proporciona la URL /auth/login/google-oauth2/
      window.location.href = `${API_URL}/auth/login/google-oauth2/`;

    } catch (error) {
      console.error('Error al iniciar autenticación con Google:', error);
      setIsAuthenticating(false);
    }
  };

  // No renderizar nada mientras se carga la configuración
  if (isLoading) {
    return null;
  }

  // No renderizar nada si Google SSO está desactivado
  if (!ssoConfig || !ssoConfig.google_sso_activo) {
    return null;
  }

  return (
    <Button
      type="button"
      variant="outline"
      className="w-full relative hover:bg-gray-50 transition-colors"
      onClick={handleGoogleSSO}
      disabled={isAuthenticating}
    >
      {isAuthenticating ? (
        <>
          <Loader2 className="mr-2 h-4 w-4 animate-spin" />
          Conectando con Google...
        </>
      ) : (
        <>
          {/* Ícono de Google */}
          <svg
            className="mr-2 h-5 w-5"
            viewBox="0 0 24 24"
            xmlns="http://www.w3.org/2000/svg"
          >
            <path
              d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
              fill="#4285F4"
            />
            <path
              d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
              fill="#34A853"
            />
            <path
              d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
              fill="#FBBC05"
            />
            <path
              d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
              fill="#EA4335"
            />
          </svg>
          Continuar con Google
        </>
      )}
    </Button>
  );
}

...
from .serializers import CategoriaServicioSerializer, ServicioSerializer, ServicioListSerializer

class CategoriaServicioListView(generics.ListCreateAPIView):
    """
    Vista para listar y crear categorías de servicios
    """
    queryset = CategoriaServicio.objects.filter(is_active=True)
...
class ServicioListView(generics.ListCreateAPIView):
    ...
    """
    queryset = Servicio.objects.filter(is_active=True).select_related('categoria')
    permission_classes = [permissions.IsAuthenticatedOrReadOnly]
